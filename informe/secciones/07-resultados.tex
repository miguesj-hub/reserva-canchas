...
A la fecha de este informe están construidas y demostrables las cuatro
...
\begin{itemize}
  \item[] \textbf{Gestión de usuarios.} El administrador lista las cuentas y las
        activa o inactiva. Una cuenta inactivada deja de entrar de inmediato,
        incluso si tenía la sesión abierta: el gateway verifica la credencial en
        cada petición, y el frontend la devuelve al inicio de sesión con el
        motivo.
\end{itemize}

Las cuatro historias del alcance están, por tanto, construidas y son
demostrables por separado.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{C{1.4cm} Y C{2cm} L{3cm}}
    ...
    \textsf{CA-6}\label{ca:6} & Al menos dos microservicios independientes sobre PostgreSQL & Cumplido & Tres con base propia y aislada por credenciales, más \id{ms-reportes} sin base (\cref{sec:estrategia-datos}). Las 70 pruebas de los cinco servicios pasan. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{Las pruebas de arranque necesitan Docker}, porque levantan un
        PostgreSQL efímero con Testcontainers. No es una carencia frente a la
        alternativa —una base instalada en la máquina—, pero sí una dependencia
        que conviene declarar. Las pruebas de las reglas de negocio, que son las
        que el Principio~II exige, no necesitan ni Docker ni Spring.
\end{itemize}
